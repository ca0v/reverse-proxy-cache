...
describe("tests proxy server", () => {
  ...
  it("tests with echo service offline", async () => {
    echo.stop();
    try {
...
describe("tests proxy server offline", () => {
  let proxy: ProxyServer | void;

  // start proxy and echo server
  before(async () => {
    config["reverse-proxy-cache"].offline = true;
    // start the proxy server
    proxy = await run(config);
  });
  ...
  it("tests offline server", async () => {
    // echo is running but should not get visitied
    const echo = new EchoServer({ port: echoPort });
    echo.start();
    try {
      ...
      assert.notStrictEqual(
        response.body,
        `echo(${key})`,
        "offline server should miss and not contact echo server"
      );
    } finally {
      echo.stop();
    }
